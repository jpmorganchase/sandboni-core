...
public class GitRepository implements GitInterface {
    ...
    public ChangeScope<Change> getChanges(String fromRev, String toRev) throws SourceControlException {
        StopWatch sw1 = StopWatchManager.getStopWatch(this.getClass().getSimpleName(), SWConsts.METHOD_NAME_GET_CHANGES, "revisionResolver.resolve").start();
        RevisionScope<ObjectId> revisionScope = revisionResolver.resolve(fromRev, toRev);
        sw1.stop();
        StopWatch sw2 = StopWatchManager.getStopWatch(this.getClass().getSimpleName(), SWConsts.METHOD_NAME_GET_CHANGES, "changeScopeResolver.getChangeScope").start();
