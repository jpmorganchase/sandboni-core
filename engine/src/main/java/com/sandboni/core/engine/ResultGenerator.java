...
public class ResultGenerator {
    ...
    public Result generate(ResultContent... resultContents) {
        StopWatch swAll = StopWatchManager.getStopWatch(this.getClass().getSimpleName(), "generate", "ALL").start();
        log.debug("....generate result for {}", Arrays.stream(resultContents).collect(Collectors.toList()));

        Arrays.stream(resultContents)
                .parallel()
                .forEach(rc -> {
                    StopWatch sw1 = StopWatchManager.getStopWatch(this.getClass().getSimpleName(), "generate", "getContent for " + rc).start();
                    ...
                    if (rc.isOutputToFile()) { //if we want to output to file
                        StopWatch sw2 = StopWatchManager.getStopWatch(this.getClass().getSimpleName(), "generate", "writeOutputToFile").start();
