...
public class JavaxControllerClassVisitor extends ClassVisitorBase implements ClassVisitor {
    ...
    public Stream<Link> start(JavaClass jc, Context c) {
        controllerPath = null;
        StopWatch sw1 = StopWatchManager.getStopWatch(this.getClass().getSimpleName(), SWConsts.METHOD_NAME_START, "getAnnotation").start();
        ...
        if (path != null) {
            StopWatch sw2 = StopWatchManager.getStopWatch(this.getClass().getSimpleName(), SWConsts.METHOD_NAME_START, "get controllerPath").start();
